function forest = forestTrain2( data, hs, varargin )
v = cell2mat(hs);
max(v(:)),
...
fprintf('beginning of treeTrain\n'),
%Gmax = cellfun(@(M) max(M(:)), hs, 'Uniform', 0);
%Gmax,
...
%  temp_hs = cell2mat(hs1);
%  n_classes = max(temp_hs(:)) + 1;
  n_classes = 30;
  gtWidth = size(hs1{1},1);
  new_hist = histc(reshape(reshape(cell2mat(hs1)',gtWidth,gtWidth,n1),gtWidth*gtWidth,n1)',0:n_classes-1);
  [~,ind] = max(new_hist);
  if (size(ind-1,2) ~= 256)
      whos ind
      size(ind-1,1)
  ...
  hsn{k} = reshape(ind-1,gtWidth,gtWidth)';
  
  %distr(k,:)=histc(hs1,1:H)/n1; [~,hsn{k}]=max(distr(k,:));
  if(discr), [hs1,~]=feval(discretize,hs1,H); hs1=uint32(hs1); end
  if(discr), assert(all(hs1>0 & hs1<=H)); end;
  pure=all(hs1(1)==hs1);
  %if(~discr), 
  %if(pure), distr(k,hs1(1))=1; hsn{k}=hs1(1); else
       % end ; end
...
f = cell2mat(hsn{1});
max(f(:)),
fprintf('end of treeTrain\n'),
%Gmax = cellfun(@(M) max(M(:)), hsn, 'Uniform', 0);
%Gmax,
end
